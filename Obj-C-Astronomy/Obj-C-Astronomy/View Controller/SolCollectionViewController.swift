...
class SolCollectionViewController: UICollectionViewController {
    ...
    let marsRoverController = PDMMarsRoverController()
    var rover: PDMRover?
    var sols: [Int32]?
    ...
    var imageCache = PDMCache(key: String(), value: Data())
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as? PDMRoverPhotoCollectionViewCell else { return UICollectionViewCell() }
        if !isLoadingPhotos {
    ...
    func fetchImage(forCell cell: PDMRoverPhotoCollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        let fetchOp = PDMFetchRoverImageOperation(imageURLString: httpsURLString)
