...
class DetailViewController: UIViewController {
    ...
    private func updateViews() {
        DispatchQueue.main.async {
            guard let photo = self.photo, self.isViewLoaded else { return }
            do {
                let data = try Data(contentsOf: photo.imgSrc.usingHTTPS!)
                self.solPhotoIV.image = UIImage(data: data)
                let dateString = self.dateFormatter.string(from: photo.earthDate)
                self.detailLabel.text = "Taken by \(photo.camera.roverId) on \(dateString) (Sol \(photo.sol))"
                self.cameraLabel.text = photo.camera.fullName
            } catch {
